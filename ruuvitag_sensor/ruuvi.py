...
class RuuviTagSensor(object):
    ...
    def convert_data(raw):
        ...
        data = RuuviTagSensor._get_data_format_7(raw)
        if data is not None:
            return (7, data)

        data = RuuviTagSensor._get_data_format_8(raw)
        if data is not None:
            return (8, data)

        data = RuuviTagSensor._get_data_format_9(raw)
        if data is not None:
            return (9, data)
    ...
    def _get_data_format_7(raw):
        ...
        Returns:
            string: Sensor data
        """
        # Search of FF990407 (Manufacturer Specific Data (FF) / Ruuvi Innovations ltd (9904) / Format 7 (07))
        try:
            if "FF990407" not in raw:
                return None

            payload_start = raw.index("FF990407") + 6
            return raw[payload_start:]
        except:
            return None
    ...
    def _get_data_format_8(raw):
        ...
        try:
            if "FF990408" not in raw:
                return None

            payload_start = raw.index("FF990408") + 6
            return raw[payload_start:]
        except:
            return None

    @staticmethod
    def _get_data_format_9(raw):
        """
        Validate that data is from RuuviTag and is Data Format 8
