...
class BleCommunicationNix(BleCommunication):
    ...
    def start(bt_device=''):
        ...
        # This section was changed by Uriel in an attempt to fix other issues. 
        subprocess.call('sudo hciconfig %s reset' % bt_device, shell=True, stdout=DEVNULL)
        hcitool = subprocess.Popen([ 'sudo', '-n', 'hcitool', '-i', bt_device, 'lescan2', '--duplicates'], stdout=DEVNULL)
    ...
    def get_datas(blacklist=[], bt_device=''):
        ...
        for line in BleCommunicationNix.get_lines(procs[1]):
            try:
                ...
                # TODO Blacklist is disabled for now by Uriel Tan
                # Reason: ble devices are added to the blacklist when they disconnect and this causes us to be not able to see the ble devices
                #if mac in blacklist:
                #    continue
                data = line[26:]
                yield (mac, data)
