...
class Df7Decoder(object):
    ...
    def decode_data(self, data):
        ...
        key = '\x80\xe6\xb3\x6d\x97\x75\xb1\x5c\x7e\x3c\xc2\xc0\x7c\x99\x20\xf6'
        encrypted = encrypted_data[1:17]
        decipher = AES.new(b'\x80\xe6\xb3\x6d\x97\x75\xb1\x5c\x7e\x3c\xc2\xc0\x7c\x99\x20\xf6', AES.MODE_ECB)
        decrypted = b'\x07'+ decipher.decrypt(encrypted)
        # print(data)
        # print(encrypted)
        # print(decrypted)
        try:
            ...
            return {
                ...
                'acceleration_z': acc_z,
                'battery': self._get_battery(byte_data),
                'sensor': self._get_sensor(byte_data)
            }
        except Exception:
            log.exception('Value: %s not valid', data)
            return None
...
class Df9Decoder(object):
    ...
    def _get_v_cap(self, data):
        '''Return battery mV'''
        return (data[14] << 8) + data[15]

    def _get_p_rssi(self, data):
        '''Return battery mV'''
        print('------------------------')
        #print(len(data))
        #print(data)
    ...
    def decode_data(self, data):
        ...
        encrypted = encrypted_data[1:17]
        decipher = AES.new(b'\x80\xe6\xb3\x6d\x97\x75\xb1\x5c\x7e\x3c\xc2\xc0\x7c\x99\x20\xf6', AES.MODE_ECB)
        byte_data_decrypted = [9]
        decrypted = decipher.decrypt(encrypted)
        for b in decrypted:
            byte_data_decrypted.append(b)
        byte_data_decrypted.append(encrypted_data[17])
        
        #print(byte_data_decrypted)
        decrypted = byte_data_decrypted 
        try:
            byte_data = decrypted
            acc_x, acc_y, acc_z = self._get_acceleration(byte_data)
            return {
                ...
                'acceleration_z': acc_z,
                'battery': self._get_battery(byte_data),
                'v_cap': self._get_v_cap(byte_data),
                'p_rssi': self._get_p_rssi(byte_data)
            }
        except Exception:
            log.exception('Value: %s not valid', data)
            return None
